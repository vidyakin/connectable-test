<template>
  <div class="structure">
    ...
    <div class="structure-header">
      <div class="structure-header-name">Структура</div>
    ...
    <a-tabs defaultActiveKey="1" @change="callback">
      <a-tab-pane tab="Структура" key="1">
        <div class="c-structure" v-if="departments">
          ...
          <div
            class="c-structure__head"
            v-for="(dep, index) in departments.filter(e => e.level === 1)"
            :key="index"
          >
            <div class="structure-el">
              <a-popover placement="bottom" trigger="click">
                <template slot="content">
                  ...
                  >
                    ...
                    >
                      <a-avatar
                        :src="(text.googleImage ? text.googleImage : require('../assets/no_image.png'))"
                      ></a-avatar>
              ...
              <a-tooltip title="Редактировать" v-if="userCanRead">
                <a-button
                  class="delete-department edit"
                  icon="edit"
                  @click="openDepEdit(dep.name, dep.users, dep._id)"
                ></a-button>
              </a-tooltip>
            </div>
          </div>
          ...
          <div class="c-structure__row">
            <div
              class="c-structure__item"
              v-for="(dep_2, index) in departments.filter(e => e.level > 1 && e.level < 3)"
              :key="index"
            >
              <div class="c-structure__article" v-if="(dep_2.level == 2)">
                <a-popover placement="bottom" trigger="click">
                  ...
                  <div class="c-structure__link" @click="currentLenth">{{dep_2.name}}</div>
                </a-popover>
                <a-popconfirm
                  title="Подтверите удаление"
                  okText="Подтверждаю"
                  cancelText="Отмена"
                  @confirm="deleteBlock(dep_2._id)"
                  v-if="userCanRead"
                >
                  <a-tooltip title="Удалить">
                    <a-button class="delete-department" icon="delete"></a-button>
                ...
                <a-tooltip title="Редактировать" v-if="userCanRead">
                  <a-button
                    class="delete-department edit"
                    icon="edit"
                    @click="openDepEdit(dep_2.name, dep_2.users, dep_2._id)"
                  ></a-button>
                </a-tooltip>
              </div>
              ...
              <div class="c-structure__child__row">
                ...
                <div
                  class="c-structure__child__item"
                  v-for="(dep_3, index) in departments.filter(e => e.level == 3 && e.parent.label == dep_2.name)"
                  :key="index"
                >
                  <div class="c-structure__child__article">
                    ...
                    <a-tooltip title="Редактировать" v-if="userCanRead">
                      <a-button
                        class="delete-department edit"
                        icon="edit"
                        @click="openDepEdit(dep_3.name, dep_3.users, dep_3._id)"
                      ></a-button>
                    </a-tooltip>
                  </div>
                  <!-- было v-if="(dep_4.level == 4 && dep_4.parent.label == dep_3.name)" -->
                  <div
                    class="c-structure__detail__row"
                    v-for="(dep_4, index) in departments.filter(e => e.level === 4 && e.parent.label === dep_3.name)"
                    :key="index"
                  >
                      ...
                      <div class="c-structure__detail__article">
                        <a-popover placement="bottom" trigger="click">
                          <template slot="content">
                            ...
                            <div
                              class="count"
                              v-if="len(dep_4.users)"
                            >{{ countString(dep_4.users) }}</div>
                          </template>
                        ...
                        <a-tooltip title="Редактировать" v-if="userCanRead">
                          <a-button
                            class="delete-department edit"
                            icon="edit"
                            @click="openDepEdit(dep_4.name, dep_4.users, dep_4._id)"
                          ></a-button>
                        </a-tooltip>
                      </div>
                    </div>
...
export default {
  ...
  methods: {
    ...
    deleteBlock(id) {
      this.$store.dispatch(DELETE_DEP, id).finally(() => {
  ...
  computed: {
    ...
    userCanRead() {
      const val =
        Boolean(this.datauser) &&
        this.$can("read", {
          accessEmail: this.datauser.email,
          __type: "Admin"
        });
      console.log(
        `userCanRead: datauser=${JSON.stringify(this.datauser)} val=${val}`
      );
      return val;
    }
  },
...
.c-structure__detail__item .delete-department,
.c-structure__child__item .delete-department {
  right: 0;
}
.is-hide-img-header {
  .structure {
